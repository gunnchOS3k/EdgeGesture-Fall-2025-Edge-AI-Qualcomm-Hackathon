#!/usr/bin/env python3
"""
Robust Racing Controller with Improved Hand Detection
"""
...
class RobustRacingController:
    def __init__(self):
        """Initialize the robust racing controller."""
        ...
        # Smoothing for grip detection
        self.left_grip_history = []
        self.right_grip_history = []
        self.grip_history_size = 5
    ...
    def detect_hands_robust(self, frame):
        """Robust hand detection with better positioning."""
        # Convert BGR to RGB
    ...
    def extract_hand_data_robust(self, hand_landmarks, width, height):
        ...
        # Multiple grip detection methods
        grip_score = self.calculate_grip_score(hand_landmarks, width, height)
        open_score = self.calculate_open_score(hand_landmarks, width, height)
        
        # Determine hand state based on scores
        is_gripping = grip_score > 0.6
        is_open = open_score > 0.6
    ...
    def calculate_grip_score(self, hand_landmarks, width, height):
        """Calculate a grip score (0-1) based on multiple factors."""
        landmarks = hand_landmarks.landmark
        ...
        # Method 3: Wrist to middle finger distance (shorter when gripping)
        wrist = landmarks[0]
        ...
        thumb_score = 1.0 if thumb_index_dist < 50 else 0.0
        wrist_score = 1.0 if wrist_middle_dist < 80 else 0.0
        
        # Weighted combination
        grip_score = (finger_score * 0.6 + thumb_score * 0.2 + wrist_score * 0.2)
        
        return grip_score
    
    def calculate_open_score(self, hand_landmarks, width, height):
        """Calculate an open palm score (0-1)."""
        landmarks = hand_landmarks.landmark
        ...
        # Method 3: Wrist to middle finger distance (longer when open)
        wrist = landmarks[0]
        middle_tip = landmarks[12]
        wrist_middle_dist = math.sqrt(
            (wrist.x - middle_tip.x)**2 + (wrist.y - middle_tip.y)**2
        ...
        thumb_score = 1.0 if thumb_index_dist > 80 else 0.0
        wrist_score = 1.0 if wrist_middle_dist > 100 else 0.0
        
        # Weighted combination
        open_score = (finger_score * 0.6 + thumb_score * 0.2 + wrist_score * 0.2)
    ...
    def smooth_grip_detection(self, left_hand, right_hand):
        ...
        if left_hand:
            ...
            self.left_hand_gripping = avg_grip > 0.6
        ...
        if right_hand:
            ...
            self.right_hand_gripping = avg_grip > 0.6
        
        # Both hands open for braking
        self.both_hands_open = (left_hand and left_hand['open_score'] > 0.6 and 
                              right_hand and right_hand['open_score'] > 0.6)
    ...
    def draw_hands_and_line(self, frame, left_hand, right_hand):
        ...
        if left_hand:
            ...
            open_score = left_hand['open_score']
        ...
        if right_hand:
            ...
            open_score = right_hand['open_score']
    ...
    def run(self):
        ...
        print("Robust Racing Controller")
        print("=======================")
        ...
        try:
            while self.running:
                ...
                # Display the frame
                cv2.imshow('Robust Racing Controller', frame)
...
def main():
    """Main entry point."""
    controller = RobustRacingController()
    controller.run()
